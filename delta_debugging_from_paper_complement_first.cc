//DD is modified to match the algorithm from the paper except the complement is tested before the subset
...
void DeltaDebugging::ddmin(std::vector<CXCursor> source, const char *srcPath,
  ...
  if (source.size() >= 1) {
	...
	if(status == SUCCESS) { //complement of subset passed the test, so reset all to the subset
	  resetCursors(level);
	  ...
	  break;
	} else {//complement didn't pass, so check the subset
